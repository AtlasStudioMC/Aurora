...
public class CanvasFileMigration {
    ...
    public static void initMigration(final WorldMigrationContext migrationContext) {
        ...
        if (!todo.isEmpty()) {
            LOGGER.info("Beginning migration of Aurora filesave features");
            ...
            LOGGER.info("All Aurora features migrated successfully, continuing with startup");
        }
    }
    ...
    private static void tryPause() {
        try {
            ...
            LOGGER.info("Continuing with Aurora file migration, please wait");
        } catch (final InterruptedException ie) {
            Thread.currentThread().interrupt();
            throw new RuntimeException("Interrupted while waiting before startup Aurora file migration", ie);
        } finally {
            MSG_SHOWN = true;
        }
    }
