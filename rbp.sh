...
process_changes "./aurora-server/src/minecraft/java" "Minecraft"

gradle_rebuild_task=false

if $gradle_run || ! git diff --quiet "./aurora-server/build.gradle.kts" || ! git diff --cached --quiet "./aurora-server/build.gradle.kts"; then
  gradle_rebuild_task=true
fi

if $gradle_run || ! git diff --quiet "./aurora-api/build.gradle.kts" || ! git diff --cached --quiet "./aurora-api/build.gradle.kts"; then
  gradle_rebuild_task=true
fi
