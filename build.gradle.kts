...
paperweight {
    ...
    upstreams.paper {
        ...
        patchFile {
            ...
            outputFile = file("aurora-server/build.gradle.kts")
            patchFile = file("aurora-server/build.gradle.kts.patch")
        }
        patchFile {
            path = "paper-api/build.gradle.kts"
            outputFile = file("aurora-api/build.gradle.kts")
            patchFile = file("aurora-api/build.gradle.kts.patch")
        }
        patchDir("paperApi") {
            upstreamPath = "paper-api"
            excludes = setOf("build.gradle.kts")
            patchesDir = file("aurora-api/paper-patches")
...
subprojects {
    ...
    if (project.name.endsWith("-debug") || project.name.endsWith("-plugin")) {
        ...
        dependencies {
            compileOnly(rootProject.projects.auroraServer) {
...
tasks.register("fixupMinecraftFilePatches") {
    dependsOn(":aurora-server:fixupMinecraftSourcePatches")
}
